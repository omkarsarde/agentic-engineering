...
Use the [dataset-card template](../artifacts/templates/dataset-card.md), in the lineage of [Datasheets for Datasets](https://arxiv.org/abs/1803.09010), for training, tuning, retrieval, and evaluation collections.
...
Use the [model-card template](../artifacts/templates/model-card.md), following [Model Cards for Model Reporting](https://arxiv.org/abs/1810.03993), for a trained, adapted, merged, or selected model.
...
Use the [ADR template](../artifacts/templates/architecture-decision-record.md), in [Nygard's formulation](https://cognitect.com/blog/2011/11/15/documenting-architecture-decisions). A useful ADR contains:
...
Use the [threat-model template](../artifacts/templates/threat-model.md). Chapter 24 frames agent security as four questions answered in order:
...
The executable calculator is [`sizing.py`](../code/appb/sizing.py); Chapter 33 develops the same back-of-envelope moves against the 45-minute interview clock. Keep units in function names and worksheet cells. The formulas are deliberately transparent approximations — their assumptions and owning chapters matter more than the constants.
...
## Timed practice and self-scoring {#app-b-timed}

The worked design studies live in Part IX: the reusable seven-move framework and
back-of-envelope toolkit in [Chapter 33](../chapters/33-sd-interview-framework.qmd), then
one chapter per problem family — enterprise RAG ([Chapter 34](../chapters/34-sd-enterprise-rag.qmd)),
the acting support agent ([Chapter 35](../chapters/35-sd-support-agent.qmd)), the serving
platform ([Chapter 36](../chapters/36-sd-serving-platform.qmd)), the eval platform
([Chapter 37](../chapters/37-sd-eval-platform.qmd)), cross-session memory
([Chapter 38](../chapters/38-sd-memory-system.qmd)), multi-agent deep research
([Chapter 39](../chapters/39-sd-deep-research.qmd)), and the multi-tenant agent platform
([Chapter 40](../chapters/40-sd-agent-platform.qmd)). Each ends with a family-skeleton
table of sibling prompts to practice against under the protocol below; the long-running
coding agent is exercised in [Chapter 32](../chapters/32-capstone.qmd).
