...
## The training stack is pinned as examples, not a recommended build {#app-c-training}
...
Four rows pin the assembly. PyTorch's [FSDP2 `fully_shard`](https://docs.pytorch.org/docs/main/distributed.fsdp.fully_shard.html) documents the per-parameter-sharding API a ZeRO-3 run depends on; [TorchTitan](https://github.com/pytorch/torchtitan) and [MaxText](https://github.com/AI-Hypercomputer/maxtext) are the PyTorch and JAX reference platforms that compose those features end to end; and DeepSeek's [DeepEP](https://github.com/deepseek-ai/DeepEP) is a current mixture-of-experts dispatch-and-combine communication library. All four are vendor-reported interfaces that move quickly: pin the commit, then revalidate layout, checkpoint portability, throughput, and fault recovery before adopting them.

Two rows record low-precision and optimizer recipes as measured results under their authors' setups — NVIDIA's [NVFP4 training paper](https://arxiv.org/abs/2509.25149) and the [Kimi K2 report](https://arxiv.org/abs/2507.20534) that introduced MuonClip. Three more record operations at scale: [MegaScale](https://arxiv.org/abs/2402.15627)'s model-FLOPs-utilization and restart figures, the [Llama 3 report](https://arxiv.org/abs/2407.21783)'s 466-interruption training snapshot, and [Decoupled DiLoCo](https://arxiv.org/abs/2604.21428)'s decentralized-training study. Treat each number as a dated existence proof, never a target to transfer to a different model and cluster.

The accelerators those runs land on are procurement inputs, not durable mechanisms. Google's [Ironwood](https://blog.google/innovation-and-ai/infrastructure-and-cloud/google-cloud/ironwood-google-tpu-things-to-know/) TPU and NVIDIA's announced [Vera Rubin](https://nvidianews.nvidia.com/news/nvidia-vera-rubin-platform) platform quote peaks under sparsity and precision conventions that are not comparable across vendors; benchmark the target model, precision, topology, collective mix, and power envelope before trusting a spec sheet. Finally, compute-threshold governance is a dated legal status rather than settled law: Executive Order 14110's 10^26-operation reporting trigger was revoked in January 2025 and has no active general successor, per the [Congressional Research Service summary](https://www.congress.gov/crs_external_products/R/HTML/R48555.web.html). Qualified counsel must recheck federal, state, export-control, and sector obligations; the broader governance surface is registered below and taught in Chapter 25.
